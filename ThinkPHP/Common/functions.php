...
function order_status_key(){
    $paramNums=func_num_args();
    $params=func_get_args();
    if($paramNums==1 && is_array($params[0])){
        $length=count($params[0]);
        $param_arr=$params[0];
        if($length==1){
            return array_key_of_value(C('order.status'),$param_arr[0]);
        }elseif($length>1){
            $out=array();
            foreach ($param_arr as $value) {
                $out[]=array_key_of_value(C('order.status'),$value);
            }
            return $out;
        }
    }else{
        if($paramNums==1){
            return array_key_of_value(C('order.status'),$params[0]);
        }elseif($paramNums>1){
            $out='';
...
//获取第一个满足条件的键名，模板调用（ordershow）
function array_key_of_value($arr,$value){
    foreach ($arr as $k => $v) {
        if($value==$v){
            return $k;
        }
