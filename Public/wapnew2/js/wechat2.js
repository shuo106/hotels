$(function(){
	...
	$(".getlocation").click(function(){
		...
		geo.get().then(function(position){
			var url=root+"/index.php/wechat/location/lat/"+position.coords.latitude+"/lng/"+position.coords.longitude;
			$.get(url,function(rs){
				if(rs.status==0){
					var city=rs.result.addressComponent.city;
					city=city.substring(0,city.length-1);
					window.location.href=root+"/index.php/wechat/hotels/city/"+city;
				}else{
					alert('获取地理位置信息失败');
				}
			},'json')
	...
	$("#getCode").click(function(){
		...
		$.get(root+'/Wap/getCode/mobile/'+mobile,function(rs){
			if(rs.status==1){
				var time = 60;
	...
	$("#findpwd").click(function(){
        ...
        $.post(root+'/index.php/wechat/findpwd',data,function(rs){
        	alert(rs.msg);
        	if(rs.status==1){
