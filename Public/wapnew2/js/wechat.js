$(function(){
		...
		geo.get().then(function(position){
			...
			$.get(url,function(rs){
				if(rs.status==0){
					...
					window.location.href=root+"/Wap/hotels/city/"+city;
				}else{
					alert('获取地理位置信息失败');
				}
			},'json')
	...
	$("#login").click(function(){
		...
		$.post(root+'/Wap/chklogin',data,function(rs){
			if(rs.status==1){
				window.location.href=root+'/Wap/center';
			}else{
				alert(rs.msg);
			}
		},'json');
	...
	$("#findpwd").click(function(){
        ...
        $.post(root+'/Wap/findpwd',data,function(rs){
        	alert(rs.msg);
        	if(rs.status==1){
        		window.location.href=root+'/Wap/login';
        	}
        },'json')
	});
});