...
/*****************************************地标管理*********************************************************/
	...
	public function upfile($fujian){
		...
		if(isset($_FILES[$fileName])){
			...
			$res = move_uploaded_file($_FILES[$fileName]['tmp_name'],$newName);
			if($res){
	...
	public function filenewName($extInfo){
		...
		$text = $_SESSION['hotel_name'].date('YmdHis').'-';
		for($i=0;$i<4;$i++){
			$text .= substr($str,mt_rand(0,strlen($str)-1),1);	
		}
		$new = "../uploads/Hotel/"."$text".'.'."$extInfo"; //9-27号修改
