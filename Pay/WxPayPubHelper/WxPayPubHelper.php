...
class Notify_pub extends Wxpay_server_pub 
{
	...
	public function sendCustomMessage($data){
		...
		$result = $this->http_post('https://api.weixin.qq.com/cgi-bin/message/custom/send?access_token='.$access_token,$this->Pay_json_encode($data));
		if ($result)
		{
	...
	public function getAccessToken(){
		$tokenurl = 'https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid='.WxPayConf_pub::APPID.'&secret='.WxPayConf_pub::APPSECRET;
		$result = $this->http_get($tokenurl);
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
	 ...
	 function Pay_json_encode($arr) {
		...
		foreach ( $arr as $key => $value ) {
			if (is_array ( $value )) { //Custom handling for arrays
				if ($is_list)
					$parts [] = $this->Pay_json_encode( $value ); /* :RECURSION: */
				else
					$parts [] = '"' . $key . '":' . $this->Pay_json_encode ( $value ); /* :RECURSION: */
			} else {
				$str = '';
				if (! $is_list)
					$str = '"' . $key . '":';
